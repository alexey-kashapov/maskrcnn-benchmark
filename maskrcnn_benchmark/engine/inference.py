...
def compute_on_dataset_depth(model, data_loader, device, bbox_aug, timer=None):
    model.eval()
    results_dict = {}
    cpu_device = torch.device("cpu")
    for image_ids, batch in enumerate(tqdm(data_loader)):
        #images, targets, image_ids = batch
        images, depths, targets, image_ids = batch
...
def inference_depth(
    ...
    inference_timer = Timer()
    total_timer.tic()
    predictions = compute_on_dataset_depth(model, data_loader, device, bbox_aug, inference_timer)
    # wait for all processes to complete before measuring the time
    synchronize()
    total_time = total_timer.toc()
    total_time_str = get_time_str(total_time)
